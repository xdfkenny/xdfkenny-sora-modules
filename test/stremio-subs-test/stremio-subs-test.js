...
async function extractStreamUrl(url) {
    ...
    try {
        ...
        const subsWorker = (async function() {
            try {
                ...
                // TEST HARNESS: v3 is the ONLY provider (no OS REST, no SCS) —
                // isolates the subtitle-source variable while streams stay
                // identical to the released module.
                const stremioResult = await safe(resolveStremioSubtitle(imdbId, isMovie ? 'movie' : 'series', season, episodeNum));
                merged = stremioResult ? {
                    subtitle: stremioResult.subtitle,
                    subtitles: stremioResult.subtitles,
    ...
    // TEST: curate one entry per language by downloading candidates and
    // rendering only the LARGEST body per language (most complete track),
    ...
    const engCurated = curatedEntries.find(function(e) { return e.lang === 'eng'; })
        || curatedEntries.find(function(e) { return _osTagIsEnglish(e.lang); });
    if (engCurated && subtitle && subtitleList.some(function(it) { return it.url === subtitle; })) {
        if (subtitle !== engCurated.url) {
            console.log('[SubTest] auto-load swapped to largest English: ' + engCurated.url.slice(0, 90));
            subtitle = engCurated.url;
            subtitleHeaders = engCurated.headers || subtitleHeaders;
        }
    }
    const subtitlePairs = [];
...
async function resolveStremioSubtitle(imdbId, type, season, episode) {
    try {
        // TEST FIX: series ride the Stremio addon path convention imdbId:s:e
        // (URL-encoded colons). v3 silently ignores ?season=&episode= query
        // params and answers with random whole-show tracks instead — verified
        // live: colon shape returns 91 correct-episode tracks vs 3 wrong ones.
        const contentId = type === 'series'
            ? imdbId + ':' + encodeURIComponent(String(season || 1)) + ':' + encodeURIComponent(String(episode || 1))
            : imdbId;
        const response = await soraFetchTimed(`${STREMIO_OPENSUBTITLES_URL}/subtitles/${type}/${contentId}.json`, {
            headers: {
                'Accept': 'application/json',
...
async function measureSubtitleBytes(url, headers) {
    try {
        ...
        // Sub-40-byte bodies are placeholders/errors, not real tracks.
        if (!body || body.length < 40) return -1;
        return body.length;
    } catch (e) {
        return -1;
    }

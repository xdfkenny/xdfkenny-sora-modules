const BASE_URL = 'https://hydrahd.ws'; // fork shipped .ru — dead: ajax 404s after the ws redirect (referer-host check)
...
// NOT a personal key â€” it is the default community token the addon itself
...
// Load marker: visible in the app's logs, so we can confirm the script actually
// loaded after a re-add (raw CDN can lag behind the pushed commit). Version
// numbers live in the manifest only, never here.
console.log('[HydraHD] module script loaded (fast tier + allSubtitles pool, mirrors are fallback-only, timer-safe)');
...
// non-2xx â€” handle every shape so callers can always .text()/.json().
function toResponseLike(value) {
    if (value && typeof value.text === 'function') {
        // Already Response-like (Node fetch / fetchv2 bridge) â€” but NOT safe to
...
async function soraFetch(url, options = {}) {
    ...
    // non-2xx or resolve with the raw body â€” never let that escape.
    try {
...
// immediately â€” keeping the module functional in BOTH environments.
...
async function extractStreamUrl(url) {
    ...
    let subtitle = '';
    let subtitleList = [];
    // Safety cap only â€” the function actually returns as soon as its awaited
    // workers resolve, NOT at this deadline. It just bounds the early-break
    // guards below so nothing runs against the app's ~40s hard kill.
    const startTime = Date.now();
    const timeLeft = function() { return 15000 - (Date.now() - startTime); };
    try {
        const fullUrl = url.startsWith('http') ? url : `${BASE_URL}${url}`;
        const response = await soraFetch(fullUrl);
        const html = await response.text();
        ...
        // Referer must be the ABSOLUTE page URL â€” the app hands series episode
        ...
        // or the selected-server variant ("iframe-server-button active") â€” both
        ...
        if (serverEntries.length === 0) {
            // Fallback: no named buttons found â€” use every embed link in order.
        ...
        // Diagnostic: when the ajax fragment comes back empty or unparsed the
        // module silently falls back to keyless hosts only. Log enough to tell
        // "endpoint returned nothing" apart from "regex didn't match".
        console.log('[HydraHD] Ajax fragment len=' + ajaxHtml.length + ' buttons=' + serverEntries.length + (ajaxHtml.length > 0 && serverEntries.length === 0 ? ' head=' + ajaxHtml.slice(0, 200).replace(/\s+/g, ' ') : ''));
        ...
        // xpass exposes mdata playlist files â€” all keyless like the site does.
        ...
        // ---- FAST TIER --------------------------------------------------------
        // Server buttons, direct keyless hosts and the subtitle providers all
        // run CONCURRENTLY. This tier alone resolves VixSrc/MoviesAPI/xpass in a
        // couple of round trips, so the common case never touches the mirrors.

        // Worker 1: every named server button, in parallel batches. Stops early
        // once we already have a few streams so we don't keep resolving buttons
        // that duplicate the keyless hosts.
        const serverWorker = (async function() {
            try {
                const BATCH = 6;
                for (let i = 0; i < serverEntries.length; i += BATCH) {
                    if (streams.length >= 3 || timeLeft() < 4000) break;
                ...
                await Promise.all(directHosts.map(async function(host) {
                    ...
                    try {
                        ...
                        if (resolved && resolved.streamUrl) {
                            ...
                            console.log('[HydraHD] keyless miss ' + host.name + ' (no stream)');
                        }
                    } catch (e) {
                        // Never silent: a thrown resolver used to look identical
                        // to "host has no stream", which hid the json() bug.
                        console.log('[HydraHD] keyless fail ' + host.name + ': ' + (e && e.message ? e.message : String(e)));
                    }
                }));
        ...
        // Subtitle providers run at the same time as the stream workers. No
        // stagger â€” on the app's JSContext timerSafe is a no-op anyway, and the
        // free APIs tolerate a single concurrent burst of three.
        let merged = null;
        ...
        // Shared mirror helper: pushes the resolved stream plus any extra mirrors
        // the resolver returned (VidSrc hands back up to 3) as their own entries.
        async function addMirror(title, origin, resolver) {
            if (streams.length >= 6 || timeLeft() < 4000) return;
            try {
                ...
                if (resolved && resolved.streamUrl) {
                    ...
                    console.log('[HydraHD] mirror miss ' + title + ' (no stream)');
                }
            } catch (e) {
                console.log('[HydraHD] mirror fail ' + title + ': ' + (e && e.message ? e.message : String(e)));
            }
        }

        // Worker 3: VidFast runs CONCURRENTLY with the fast tier. Its enc/dec
        // handshake is only a few round trips, so awaiting it alongside the
        // other workers costs no extra wall time unless it is the slowest one â€”
        // and it needs to be here for the third server to appear at all.
        ...
        // ---- FALLBACK TIER ----------------------------------------------------
        // VidSrc stays gated: it POLLS a sources API, and on Shirox the between-
        // poll waits are no-ops, so it fires its iterations back to back. That
        // was the main reason stream resolution dragged. Only pay for it when
        // everything above came up short.
        async function runVidSrcFallback() {
        ...
        // Await the fast tier + VidFast + subtitles. A worker failure must NEVER
        // abort the whole result (that was killing streams even when they
        // succeeded).
        await Promise.allSettled([serverWorker, keylessWorker, vidfastWorker, subsWorker]);

        // Pay the VidSrc poll cost only when nothing else produced enough.
        if (streams.length < 2 && timeLeft() > 4000) {
            try { await runVidSrcFallback(); } catch (e) {
        ...
        // SUBTITLES rendition down to its real .vtt file â€” the playlist URI is
        // just an HLS wrapper around one WebVTT segment. All languages surface
        // in the pool; providers above keep priority for English. Skipped
        // entirely when the deadline is close â€” never delay the return.
        try {
            const primaryHls = streams.length > 0 ? streams[0].streamUrl : null;
            if (primaryHls && /\.m3u8|playlist/i.test(primaryHls) && timeLeft() > 3000) {
    ...
    // One clean entry per language (English first) for the in-player subtitle
    // pool. `allSubtitles` is what Shirox reads to fill its subtitle menu, so
    // there's no separate picker screen. `subtitles` stays a single default URL
    // (Sora auto-loads it, no picker), and `subtitlesHeaders` carries any header
    // the default track needs (keyless providers need none).
    ...
    // Only auto-load an English default; a foreign-only pool stays manual so the
    // player doesn't silently open in a language the user can't read.
    if (subtitle) {
        streams.forEach(function(stream) {
            if (stream && !stream.subtitle) stream.subtitle = subtitle;
        });
    }
    const subsLog = allSubtitles.map(function(s) { return s.label; }).join(',');
    console.log('[HydraHD] Return summary streams=' + streams.length + ' primary=' + (primaryStream ? primaryStream.slice(0, 120) : 'null') + ' subtitle=' + (subtitle ? subtitle.slice(0, 120) : 'null') + ' subtitleCount=' + allSubtitles.length + ' subs=[' + subsLog + ']');
    return JSON.stringify({
...
// with the x-player-key header â†’ response.sources[0].url is the master.m3u8.
...
// â†’ {"src":"/embed/{id}?token=...&expires=..."} â†’ fetch the embed page and
// read window.masterPlaylist.params (token/expires) â†’ playlist URL resolves
...
// Whiskey: the embed page leaks mdata file ids â†’ GET https://play.xpass.top/
// mdata/{id}/1/playlist.json â†’ {playlist:[{sources:[{file:"...master.m3u8"}]}]}
...
// one {url, lang, label, headers} entry per track (all languages the playout
// offers). Wrappers are resolved in parallel â€” sequentially this was up to
// 8 back-to-back fetches and a big chunk of the perceived stall.
async function extractPlaylistSubtitleTracks(masterUrl) {
    ...
    try {
        ...
        while ((line = mediaRe.exec(text)) !== null) {
            ...
            if (renditions.length >= 8) break;   // bound the wrapper downloads
            ...
            renditions.push({ name: name, langCode: langCode, wrapperUrl: wrapperUrl });
        }
        const resolved = await Promise.all(renditions.map(async function(ren) {
            try {
                const wr = await soraFetchTimed(ren.wrapperUrl, {
                    headers: { 'Referer': 'https://vixsrc.to/', 'Accept': 'application/vnd.apple.mpegurl' }
                }, 5000);
                if (!wr) return null;
                const wt = await wr.text();
                const vttMatch = wt.match(/https?:\/\/[^\s"'<>]+\.vtt[^\s"'<>]*/);
                if (!vttMatch) return null;
                return {
                    url: vttMatch[0],
                    lang: (ren.langCode || '').toLowerCase() || (ren.name || '').toLowerCase().slice(0, 3),
                    label: ren.name || 'Subtitle',
                    headers: { 'Referer': 'https://vixsrc.to/' }
                };
            } catch (e) { return null; }
        }));
        resolved.forEach(function(t) { if (t) tracks.push(t); });
...
async function resolveStremioSubtitle(imdbId, type, season, episode) {
    try {
        ...
        // Every language the provider offers goes into the pool so the user can
        // choose. English stays the auto-load default: the first real English
        // track (skipping forced/signs-only variants). No subtitle downloads â€”
        // the list and auto-load ride on provider metadata alone, so the pool
        // always appears fast.
...
// a shared public identityToken â€” no personal API key, no daily quota, no login.
...
async function resolveCommunitySubtitle(imdbId, type, season, episode) {
    ...
    try {
        ...
        // Subtitle" placeholder â€” drop those from the list entirely.
        const subtitles = ((data && data.subtitles) || [])
        ...
        // Every language the provider offers goes into the pool. English is the
        // auto-load default (first real English track, skipping forced/signs-only
        // variants). No subtitle downloads â€” the list rides on provider metadata
        // alone, so it always appears.
        const english = subtitles.filter(isEnglishStremioSubtitle);
        const preferred = english.find(function(s) {
            return !/forced|signs|sdh|hi\b/i.test(String(s.label || ''));
        }) || english[0];
...
// sublanguageid-eng and offer every returned track in the pool.
...
async function resolveOsRestSubtitle(imdbId, isMovie, season, episode) {
    ...
    try {
        ...
        const response = await soraFetchTimed(url, { headers }, 5000);
        if (!response) return null;
        const data = await response.json().catch(function() { return null; });
        ...
        // Every English track the API returned goes into the pool, auto-loading
        // the first one. No verification downloads â€” a dead subtitle API must
        // never make the list disappear.
        const result = {
...
// Merge the three keyless providers' results into one pool. OS REST English
// entries come first (full OpenSubtitles set for series AND movies), v3 and SCS
// fill in every language the providers offer. URLs are deduplicated so the same
// file never appears twice.
...
// Preferred language order for the curated subtitle pool. English is always
...
// Human-readable names for the subtitle pool.
const SUB_LANG_NAMES = {
    ...
    per: 'Persian', fas: 'Persian', slo: 'Slovak', scc: 'Serbian', scr: 'Croatian',
...
// Per-entry headers (e.g. the vixsrc Referer on playlist .vtt tracks) are
// carried through so allSubtitles entries fetch correctly.
function curatedSubtitleEntries(subtitleList) {
    ...
    subtitleList.forEach(function(item) {
        ...
        if (!existing || (isUtf8 && existing.url.indexOf('senc=') !== -1)) {
            byLang[lang] = { url: url, headers: item.headers || {} };
        }
    });
...
async function resolveVidSrc(imdbId, isMovie = true, season = null, episode = null, budgetMs = 30000) {
    ...
    // Poll ceiling cut from 15 to 4: on the app's JSContext the between-poll
    // timerSafe is a no-op, so 15 iterations meant 15 rapid back-to-back API
    // calls â€” the biggest single contributor to the Shirox stall when the
    // mirror ran. Four is enough for the sources API to warm up.
    for (let i = 0; i < 4; i++) {
...
async function verifyStreamUrl(streamUrl) {
    try {
        // A .m3u8 URL is HLS by construction â€” trust the extension.
        ...
        // serve the playlist under a .txt/.m3u8-less URL â€” accept only when

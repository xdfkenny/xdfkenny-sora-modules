...
function fillManifest(entry, m) {
  ...
  if (m.scriptUrl || m.scriptURL) addLink('script.js', m.scriptUrl || m.scriptURL);
  ...
  btnCopyJs.onclick = () => {
    ...
    if (sUrl) copyText(sUrl).then(() => flashCopied(btnCopyJs, 'Copied!'));
  };
...
async function testModule(entry) {
  ...
  await addCheck('Required manifest fields', async () => {
    ...
    const sUrlChk = m.scriptUrl || m.scriptURL || '';
    if (!/^https:\/\//.test(String(sUrlChk || ''))) warns.push('scriptUrl is not https');
    if (typeof m.asyncJS !== 'boolean') warns.push('asyncJS missing (assumed false)');
    // allow scriptURL as alias for scriptUrl
    if (m.scriptURL && !m.scriptUrl) {
      const idx = missing.indexOf('scriptUrl');
  ...
  /* 3 · script */
  await addCheck('Script · ' + shortUrl(m && (m.scriptUrl || m.scriptURL) || ''), async () => {
    const sUrl = m && (m.scriptUrl || m.scriptURL);
    if (!m || !sUrl) return { ok: false, detail: 'Manifest has no scriptUrl' };
...
async function boot() {
  ...
  try {
    ...
    // Fallback: try raw GitHub (covers file:// open and local CORS issues)
    try {
